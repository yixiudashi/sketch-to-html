...
class CommonLayer {
    ...
    getHtml (childString) {
        ...
        if (layer.style.backgroundImage) {
            return `<img id="${layer.id}" class="${layer.name} layer_type_${layer.type}" src="${path.join(this.imagePath, layer.style.backgroundImage)}.png" style="${util.getStyleString({ ...layer.finalStyle, 'background-image': undefined })}" />`;
        }
        return `<div id="${layer.id}" class="${layer.name} layer_type_${layer.type}" style="${util.getStyleString(layer.finalStyle)}" >${childString}</div>`;
    }
}
